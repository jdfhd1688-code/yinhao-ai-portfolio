...
export function ProjectMediaPlaceholder({ title, subtitle, status, mediaType, aspectRatio }: Omit<PlannedAsset, "path"> & { plannedPath?: string }) {
  return (
    <article className="planned-media" style={{ aspectRatio }} role="note" aria-label={`${title}, ${status.replaceAll("_", " ").toLowerCase()}`}>
      ...
      <div className="planned-media__copy"><strong>{title}</strong><p>{subtitle}</p></div>
    </article>
  );
}
