...
export function ShortDramaCase() {
  ...
  return (
    <main className="case drama-case"><Navigation />
      ...
      <section className="selected-dramas numbered-section"><span>03 / {zh ? "两部作品" : "SELECTED WORKS"}</span>{dramaWorks.map((work, index) => <article key={work.id}><div className="drama-work__index">WORK 0{index + 1}</div><h2>{work.title}</h2><p>{zh ? "备选名" : "Alternate title"} · {work.altTitle}</p>{work.source && <p className="drama-source">{zh ? "原 IP" : "Source IP"} · {work.source[locale]}{work.sourceUrl ? <> · <a href={work.sourceUrl} target="_blank" rel="noopener noreferrer">{zh ? "查看原著" : "View source"}</a></> : null}</p>}<div className="drama-facts">{work.facts[locale].map((fact) => <span key={fact}>{fact}</span>)}</div><blockquote>{work.engine[locale]}</blockquote><ul>{work.decisions[locale].map((decision) => <li key={decision}>{decision}</li>)}</ul><a href={work.fullWatchUrl || work.watchUrl} target="_blank" rel="noopener noreferrer">{zh ? "观看已发行作品" : "WATCH RELEASED WORK"} <ArrowUpRight size={17} /></a><small>{zh ? "移动端打开体验更佳" : "Best viewed on mobile."}</small></article>)}</section>
